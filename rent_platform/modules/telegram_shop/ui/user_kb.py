...
# CategoriesRepo optional (project step-by-step)
try:
...
async def _send_categories_menu(bot: Bot, chat_id: int, tenant_id: str) -> None:
    ...
    if not cats:
        await bot.send_message(chat_id, "🛍 *Каталог*\n\nПоки що немає категорій.", parse_mode="Markdown")
        return
...
async def _build_product_card(tenant_id: str, product_id: int, category_id: int | None) -> dict | None:
    ...
    kb = product_card_kb(product_id=pid, has_prev=bool(prev_p), has_next=bool(next_p), category_id=category_id)
...
async def _send_first_product_card(
    ...
    *,
    ...
    if not p:
        # якщо пусто в категорії — покажемо категорії (щоб не було "нічого і все")
        await bot.send_message(
            chat_id,
    ...
    card = await _build_product_card(tenant_id, int(p["id"]), category_id)
    if not card:
...
async def _edit_product_card(
    ...
    card = await _build_product_card(tenant_id, product_id, category_id)
    if not card:
        return False
...
async def handle_update(tenant: dict, data: dict[str, Any], bot: Bot) -> bool:
        ...
        if action == "prev" and pid > 0:
            ...
            if not p:
                if cb_id:
                    await bot.answer_callback_query(cb_id, text="•", show_alert=False)
                return True
            await _edit_product_card(bot, chat_id, msg_id, tenant_id, int(p["id"]), category_id)
            if cb_id:
                await bot.answer_callback_query(cb_id)
            return True
    ...
    if text == _normalize_text(BTN_CATALOG):
        # now catalog opens categories if available
        if CategoriesRepo is not None:
            has_cats = await CategoriesRepo.has_any(tenant_id)  # type: ignore[misc]
            if has_cats:
                await _send_categories_menu(bot, chat_id, tenant_id)
                return True
        # fallback: old behavior
        await _send_first_product_card(bot, chat_id, tenant_id, is_admin=is_admin, category_id=None)
        return True
